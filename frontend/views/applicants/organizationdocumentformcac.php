							...
							<?php $form = ActiveForm::begin(['id' => 'organization_document_form']); ?>
								...
								<div class="row" style="margin-top:20px">
											<div class="col-md-12 form-area">
												...
												<div class="box-label">CAC Certificate </div>
												
												
												<?= $form->field($model, 'document_type')->hiddenInput(['value' => 'cac certificate'])->label(false); ?>
...
<?
	$createuploaddocumentcacUrl = Url::to(['applicants/processorganizationdocumentformcac']);
	$loadc02details = Url::to(['applicants/organizationdocumentformc02']);
	$biodataform = <<<JS
  ...
  $(document).find("#customFile").addClass("selected").html('fileName');
    ...
    $.ajax({
        url: '$createuploaddocumentcacUrl',
        type: 'POST',
        data: formData,
        datatype:'json',
        // async: false,
        ...
        success: function (data) {
        	newData = data.data
			if(data.status == 1){
				$(document).find('#renderapplicationform').load('$loadc02details'+'&id='+data.data.applicant_id);
